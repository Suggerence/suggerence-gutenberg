...
namespace SuggerenceGutenberg\Components\Ai\Providers\OpenAI\Support;
...
class StructuredModeResolver
{
    public static function forModel($model)
    {
        if (self::unsupported($model)) {
            throw new Exception(sprintf('Structured output is not supported for %s', $model));
        }
    ...
    protected static function supportsStructuredMode($model)
    {
    ...
    protected static function supportsJsonMode($model)
    {
    ...
    protected static function unsupported($model)
    {
        return in_array($model, [
            'o1-mini',
            'o1-mini-2024-09-12',
