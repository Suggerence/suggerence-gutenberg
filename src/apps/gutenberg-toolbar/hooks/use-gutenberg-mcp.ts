...
export const useGutenbergMCP = (): UseGutenbergMCPTools => {
    ...
    const [isGutenbergServerReady, setIsGutenbergServerReady] = useState(false);
    ...
    useEffect(() => {
        ...
        if (gutenbergServer) {
            setIsGutenbergServerReady(true);
        }
    }, [serverConnections]);
    ...
    const getGutenbergTools = async (): Promise<SuggerenceMCPResponseTool[]> => {
        ...
        try {
            ...
            console.log('Suggerence Debug: Gutenberg tools generated:', tools);
